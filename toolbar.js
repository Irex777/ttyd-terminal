(function(){
  ...
  tb.id='mtb';
  tb.style.cssText='position:fixed;bottom:0;left:0;right:0;background:#1a1a2e;border-top:1px solid #333;z-index:9999;padding:4px;';
  var r1=document.createElement('div');
  r1.style.cssText='display:flex;gap:2px;justify-content:center;';
  var r2=document.createElement('div');
  r2.style.cssText='display:flex;gap:2px;justify-content:center;margin-top:2px;';
  var mod=null;
  function mk(label,handler,color){
    var b=document.createElement('button');
    b.textContent=label;
    b.style.cssText='flex:1;min-width:38px;height:34px;border:1px solid #333;border-radius:4px;font-size:11px;font-family:monospace;cursor:pointer;'+(color||'background:#16213e;color:#0f0;');
    b.addEventListener('click',function(e){e.preventDefault();handler(b);b.blur();});
    return b;
  }
  function send(d){if(window.term&&window.term.paste)window.term.paste(d);}
  function flash(b){var o=b.style.background;b.style.background='#e94560';b.style.color='#fff';setTimeout(function(){b.style.background=o;b.style.color='#0f0';},150);}
  ...
  var tilde=mk('~',function(b){send('~');flash(b);});r2.appendChild(tilde);
  var pipe=mk('|',function(b){send('|');flash(b);});r2.appendChild(pipe);
  var slash=mk('/',function(b){send('/');flash(b);});r2.appendChild(slash);
  var dq=mk('"',function(b){send('"');flash(b);});r2.appendChild(dq);
  tb.appendChild(r1);
  tb.appendChild(r2);
  document.body.appendChild(tb);
  var pad=document.createElement('div');
  pad.style.height=tb.offsetHeight+'px';
